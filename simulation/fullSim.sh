...
do
  ...
  PRSice_linux --base ${PREFIX}.Y${i}.glm.linear.filtered --score avg --target ${PREFIX} --out ${PREFIX}.Y${i} --A1 A1 --A2 ALT --stat BETA --snp ID --bp POS --pvalue P --keep-ambig --pheno ${PREFIX}.pheno --pheno-col Y${i}
done

awk -F'\t' 'NR==1 || $7 == "ADD" {print}' ${PREFIX}.Z.glm.linear > ${PREFIX}.Z.glm.linear.filtered
PRSice_linux --base ${PREFIX}.Z.glm.linear.filtered --score avg --target ${PREFIX} --out ${PREFIX}.Z --A1 A1 --A2 ALT --stat BETA --snp ID --bp POS --pvalue P --keep-ambig --pheno ${PREFIX}.pheno --pheno-col Z
